...
fn tensor_tensor_1() {
    ...
    let t2 = DoubleTensor::new(vec![-2.0, 1.0, 8.0,  0.0, -1.0, 2.0]);
    let t = t1 * t2;
    assert!(t == DoubleTensor::new(vec![0.0, 3.0, 16.0, 0.0, 3.0, 0.0]));
}
...
fn tensor_scalar_1() {
    let t1 = DoubleTensor::new(vec![ 0.0, 3.0, 2.0, 10.0, -3.0, 0.0]);
    let t = t1 * 3.0;
    assert!(t == DoubleTensor::new(vec![0.0, 9.0, 6.0, 30.0, -9.0, 0.0]));
}
