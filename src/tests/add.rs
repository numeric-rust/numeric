macro_rules! add_impl {
    ($t:ty, $m:ident) => (
        mod $m {
            ...
            fn tensor_tscalar_1() {
                let t1 = T::new(vec![ 0.0, 3.0, 2.0, 10.0, -3.0, 0.0]).reshape(&[2, 3]);
                let t2 = T::new(vec![-2.0]).reshape(&[2, 3]);
                let t = t1 + t2;
                assert!(t == T::new(vec![-2.0, 1.0, 0.0, 8.0, -5.0, -2.0]).reshape(&[2, 3]));
            }
            ...
            */
            ...
            #[test]
            fn tensor_scalar_2() {
                let t1 = DoubleT::new(vec![ 0.0, 3.0, 2.0, 10.0, -3.0, 0.0]).reshape(&[2, 3]);
                let t = &t1 + 3.0;
                println!("{:?}", t1.shape());
                assert!(t == DoubleT::new(vec![3.0, 6.0, 5.0, 13.0, 0.0, 3.0]).reshape(&[2, 3]));
            }
            */
        }
    )
