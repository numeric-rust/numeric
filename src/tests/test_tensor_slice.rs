...
#[test]
fn test_tensor_slice_1() {
    let t = Tensor::range(200).reshaped(&[2, 20, 5]);
    let t2 = t.slice(&[AxisIndex::SliceFrom(1),
                       AxisIndex::SliceTo(5),
                       AxisIndex::Slice(2, 4)]);
    let answer = Tensor::new(vec![102.0, 103.0, 107.0, 108.0, 112.0,
                                  113.0, 117.0, 118.0, 122.0, 123.0]).reshaped(&[1, 5, 2]);
    assert!(t2 == answer);
}

#[test]
fn test_tensor_slice_2() {
    let t = Tensor::range(210).reshaped(&[7, 5, 2, 3]);
    let t2 = t.slice(&[AxisIndex::Slice(1, 3),
                       AxisIndex::SliceTo(2),
                       AxisIndex::Full,
                       AxisIndex::Slice(1, 2)]);
    let answer = Tensor::new(vec![31.0, 34.0, 37.0, 40.0,
                                  61.0, 64.0, 67.0, 70.0]).reshaped(&[2, 2, 2, 1]);
    assert!(t2 == answer);
}

#[test]
fn test_tensor_slice_index_1() {
    let t = Tensor::range(210).reshaped(&[7, 5, 2, 3]);
    let t2 = t.slice(&[AxisIndex::Index(2),
                       AxisIndex::SliceTo(2),
                       AxisIndex::Full,
                       AxisIndex::Index(2)]);
    let answer = Tensor::new(vec![62., 65., 68., 71.]).reshaped(&[2, 2]);
    assert!(t2 == answer);
}

#[test]
fn test_tensor_slice_ellipsis() {
    let t = Tensor::range(3*2*3*7).reshaped(&[3, 2, 3, 7]);
    let t2 = t.slice(&[AxisIndex::Index(1),
                       AxisIndex::Ellipsis,
                       AxisIndex::SliceFrom(5)]);
    let answer = Tensor::new(vec![47., 48., 54., 55.,  61., 62.,
                                  68., 69., 75., 76.,  82., 83.]).reshaped(&[2, 3, 2]);
    assert!(t2 == answer);
}

#[test]
fn test_tensor_slice_implied_full() {
    let t = Tensor::range(3*2*7*3).reshaped(&[3, 2, 7, 3]);
    let t2 = t.slice(&[AxisIndex::Index(1),
                       AxisIndex::Index(1),
                       AxisIndex::Slice(1, 3)]);
    let answer = Tensor::new(vec![66., 67., 68.,
...
#[test]
#[should_panic]
fn test_tensor_slice_ellipsis_multiple() {
    let t = Tensor::range(3*2*3*7).reshaped(&[3, 2, 3, 7]);
...
#[test]
fn test_tensor_slice_new_axis_empty() {
...
#[test]
fn test_tensor_slice_new_axis_multidimensional() {
    let t = Tensor::range(210).reshaped(&[7, 5, 2, 3]);
