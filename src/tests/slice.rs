macro_rules! add_impl {
    ($t:ty, $m:ident) => (
        mod $m {
            ...
            #[test]
            fn slice_1() {
                let t = T::range(200).reshaped(&[2, 20, 5]);
            ...
            #[test]
            fn slice_2() {
                let t = T::range(210).reshaped(&[7, 5, 2, 3]);
            ...
            fn slice_index_1() {
                ...
                let t2 = t.slice(&[AxisIndex::Index(2),
                                   ...
                                   AxisIndex::Full,
                                   AxisIndex::Index(2)]);
                let answer = T::new(vec![62., 65., 68., 71.]).reshaped(&[2, 2]);
                assert!(t2 == answer);
            }

            #[test]
            fn slice_ellipsis() {
                let t = T::range(3*2*3*7).reshaped(&[3, 2, 3, 7]);
            ...
            fn slice_implied_full() {
                let t = T::range(3*2*7*3).reshaped(&[3, 2, 7, 3]);
                let t2 = t.slice(&[AxisIndex::Index(1),
                                   AxisIndex::Index(1),
                                   AxisIndex::Slice(1, 3)]);
